				...
				<div class="right-block">
					...
					<div class="content_price">
						{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
							...
							{if $product.pwyw_price == 1}<span>{l s='(Pricing suggestion only)'}</span>{/if}
							{if isset($product.specific_prices) && $product.specific_prices && isset($product.specific_prices.reduction) && $product.specific_prices.reduction > 0}
								{hook h="displayProductPriceBlock" product=$product type="old_price"}
								<span class="old-price product-price">
									{displayWtPrice p=$product.price_without_reduction}
					...
					<div class="button-container">
						{if $product.pwyw_price == 1}
							<span class="button" style="background:none;cursor:default;">{l s='Pay What You Want'}</span>
						{elseif ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.customizable != 2 && !$PS_CATALOG_MODE}
