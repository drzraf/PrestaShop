...
class CartCore extends ObjectModel
{
	...
	public function getProducts($refresh = false, $id_product = false, $id_country = null)
	{
		...
		$sql->select('cp.`id_product_attribute`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, pl.`name`, p.`is_virtual`,
						...
						product_shop.advanced_stock_management, ps.product_supplier_reference supplier_reference, IFNULL(sp.`reduction_type`, 0) AS reduction_type, IF(product_shop.`pwyw_price`=1 AND cp.`pwyw_price` IS NOT NULL, cp.`pwyw_price`, NULL) AS pwyw_price');
      ...
      $cart_quantity_multiplier = is_numeric($row['pwyw_price']) ? 1 : $row['cart_quantity'];
			...
			switch (Configuration::get('PS_ROUND_TYPE'))
			{
				case Order::ROUND_TOTAL:
					$row['total'] = $row['price'] * $cart_quantity_multiplier;
					$row['total_wt'] = $tax_calculator->addTaxes($row['price']) * $cart_quantity_multiplier;
					break;
				case Order::ROUND_LINE:
					$row['total'] = Tools::ps_round($row['price'] * $cart_quantity_multiplier, _PS_PRICE_COMPUTE_PRECISION_);
					$row['total_wt'] = Tools::ps_round($tax_calculator->addTaxes($row['price']) * $cart_quantity_multiplier, _PS_PRICE_COMPUTE_PRECISION_);
					break;

				case Order::ROUND_ITEM:
				default:
					$row['total'] = Tools::ps_round($row['price'], _PS_PRICE_COMPUTE_PRECISION_) * $cart_quantity_multiplier;
					$row['total_wt'] = Tools::ps_round($tax_calculator->addTaxes($row['price']), _PS_PRICE_COMPUTE_PRECISION_) * $cart_quantity_multiplier;
					break;
			}
		...
		foreach ($products as $product) // products refer to the cart details
		{
			...
			switch (Configuration::get('PS_ROUND_TYPE'))
			{
				case Order::ROUND_TOTAL:
					...
					if ($ecotax)
						$ecotax_total += $ecotax * (int)$cart_quantity_multiplier;
					break;
				case Order::ROUND_LINE:
					...
					if ($ecotax)
					{
						$ecotax_price = $ecotax * (int)$cart_quantity_multiplier;
				...
				default:
					...
					$products_total[$id_tax_rules_group] += Tools::ps_round($product_price, _PS_PRICE_COMPUTE_PRECISION_) * (int)$cart_quantity_multiplier;

					if ($ecotax)
					{
						$ecotax_price = $with_taxes ? $ecotax_tax_calculator->addTaxes($ecotax) : $ecotax;
						$ecotax_total += Tools::ps_round($ecotax_price, _PS_PRICE_COMPUTE_PRECISION_) * (int)$cart_quantity_multiplier;
					}
					break;
			}
		}
